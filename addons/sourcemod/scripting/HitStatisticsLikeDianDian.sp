...
DisPlayTankStatistic()
{
	...
	for (new i = 1; i<= MaxClients; i++)
	{
		if(IsClientInGame(i) && GetClientTeam(i) == TEAM_SURVIVORS)
		{
			Format(line, sizeof(line), "{green}[拳头 {red}%d {green}][石头 {red}%d {green}] {olive}%N", iTankClaw[i], iTankRock[i], i);
			CPrintToChatAll(line);
		}
	}
}
...
public Action:Event_TankKilled(Handle:event, const String:name[], bool:dontBroadcast)
{
	new Client = GetClientOfUserId(GetEventInt(event, "userid"));
    decl String:line[256];
    Format(line, sizeof(line), "{olive}Tank Alive Time: {green}%d {olive}s", AliveTime);
    CPrintToChatAll(line);
	DisPlayTankStatistic();
	iAliveTime[Client] = 0;
    AliveTime = 0;
