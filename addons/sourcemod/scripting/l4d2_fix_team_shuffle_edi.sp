...
public void MovePlayerToTeam(int client, int team)
{
	...
	switch (team)
	{
		...
		{
			if (CheckIsPlayersConnecting() == true)
			{
				ChangeClientTeam(client, L4D2_TEAM_SPECTATOR);
				CPrintToChat(client, "[{olive}fixteam{default}]{orange}有玩家加载中!请在所有玩家加载完毕后再按M加入游戏!");
				CPrintToChatAll("[{olive}fixteam{default}]{orange}玩家 %N 因有玩家加载中而无法加入游戏!其他玩家加载完后他就可以按M加入游戏了!", client);
			}
		}
...
public bool CheckIsPlayersConnecting()
{
