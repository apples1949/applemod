...
// 趣文显示参数.
//   有 l4d2_scripted_hud: HUD 只轮播"本回合趣文"(每条 FUNFACT_SWITCH_INTERVAL 秒, 实际由 scripted_hud 的
//                          FUNFACT_FACT_INTERVAL 控制, 这里只用于日志/提示文案),
//                          聊天框只补发一条"全场趣文".
//   没有 HUD:             聊天框轮播"本回合 + 全场"趣文, 每 FUNFACT_CHAT_INTERVAL 秒一条.
// HUD 轮播窗口: 实测"回合末推送 -> 下一回合开始"是 16~23 秒(见 2026-09-23 日志),
// 8 秒窗口会整段落在记分板/readyup 遮挡期里(玩家只看到 4~5 秒), 所以放宽到 22 秒:
// 前面被遮挡的那段照旧看不见, 但尾巴会延续进新回合开局, 实际可见时间因此变长.
#define FUNFACT_HUD_SHOW_TIME	22.0							// HUD 轮播窗口总时长(从推送时刻起算).
#define FUNFACT_SWITCH_INTERVAL	1.5								// 单条显示时长(与 scripted_hud 的 FUNFACT_FACT_INTERVAL 对齐, 仅用于文案).
...
// 趣文链路诊断日志: 单独的日志文件(与控制台/通用日志分开), 开关 sm_funfact_debug 默认关(需要排障时置 1).
...
public void OnPluginStart()
{
	...
	// 趣文链路专用诊断开关(默认关, 排障时 sm_funfact_debug 1): 只写专用文件 addons/sourcemod/logs/l4d2_funfact.log.
	...
	if (args) {
		...
		if (StrEqual(sArg, "help", false) || StrEqual(sArg, "?", false)) {
			...
			if (IS_VALID_INGAME(client)) {
				PrintToChat(client, "\x01用法: /stats [<类型>] [\x05round\x01/\x05game\x01/\x05team\x01/\x05all\x01/\x05other\x01]");
				PrintToChat(client, "\x01 或: /stats [<类型>] [\x05r\x01/\x05g\x01/\x05t\x01/\x05a\x01/\x05o\x01]");
				PrintToChat(client, "\x01 其中 <类型> 为 '\x04mvp\x01'、'\x04skill\x01'、'\x04ff\x01'、'\x04acc\x01' 或 '\x04inf\x01'。(更多见控制台)");
			}
...
Action Cmd_Cookie_SetPrintFlags(int client, int args)
{
	...
	if (args <= 0) {
		PrintToChat(client, "\x01用法: \x04/stats_auto <flags>\x01. 输入 \x04/stats_auto help\x01 查看更多信息。");
		return Plugin_Handled;
	}
	...
	if (StrEqual(sArg, "?", false) || StrEqual(sArg, "help", false)) {
		PrintToChat(client, "\x01用法: \x04/stats_auto <flags>\x01. flags 为回合结束时要显示的所有输出的整数和。");
		PrintToChat(client, "\x01设为 0 使用服务器自动打印默认值；设为 -1 则什么都不显示。");
		PrintToChat(client, "\x01标志列表见: \x05https://github.com/Tabbernaut/L4D2-Plugins/blob/master/stats/README.md\x01");
		return Plugin_Handled;
	} 
	
	if (StrEqual(sArg, "test", false) || StrEqual(sArg, "preview", false)) {
		if (g_iCookieValue[client] < 1) {
			PrintToChat(client, "\x01统计预览: 未设置标志。请先用 \x04/stats_auto <flags>\x01 设置标志，输入 \x04/stats_auto help\x01 查看更多信息。");
			return Plugin_Handled;
		}
	...
	if (iFlags < -1) {
		PrintToChat(client, "统计设置: 无效值: '%s'。输入 '/stats_auto help' 查看更多信息。", sArg);
		return Plugin_Handled;
	}

	if (iFlags == -1) {
		PrintToChat(client, "\x01统计设置: \x04回合结束不打印任何内容\x01。");
	} else if (iFlags == 0) {
		PrintToChat(client, "\x01统计设置: \x04使用服务器默认值\x01。");
	} else {
...
void DisplayStats(int client = -1, int round = -1, bool bTeam = true, int iTeam = -1)
{
	...
	if (g_bGameStarted) {
		...
		strcopy(strTmp, sizeof(strTmp), " (not started)");
	}
...
void GetMVPChatString(char[] printBuffer, const int iLen, bool bRound = true, bool bTeam = true, int iTeam = -1)
{
	...
	if (mvp_SI == -1 && mvp_Common == -1 && !(iBrevityFlags & BREV_SI && iBrevityFlags & BREV_CI)) {
		FormatEx(tmpBuffer, sizeof(tmpBuffer), "[MVP%s]: （行动还太少）\n", (bRound) ? "" : " - 全场");
		StrCat(printBuffer, iLen, tmpBuffer);
	} else {
		if (!(iBrevityFlags & BREV_SI)) {
			if (mvp_SI > -1) {
				if (iBrevityFlags & BREV_PERCENT) {
					FormatEx(tmpBuffer, sizeof(tmpBuffer), "[MVP%s] 特感:\x03 %s \x01(\x05%d \x01伤害,\x05 %d \x01击杀)\n",
							(bRound) ? "" : " - 全场",
					...
					FormatEx(tmpBuffer, sizeof(tmpBuffer), "[MVP%s] 特感:\x03 %s \x01(伤害 \x04%i%%\x01, 击杀 \x04%i%%\x01)\n",
							(bRound) ? "" : " - 全场",
							g_sPlayerName[mvp_SI],
							RoundFloat((bRound) ?
					...
					FormatEx(tmpBuffer, sizeof(tmpBuffer), "[MVP%s] 特感:\x03 %s \x01(\x05%d \x01伤害[\x04%i%%\x01],\x05 %d \x01击杀[\x04%i%%\x01])\n",
							(bRound) ? "" : " - 全场",
							g_sPlayerName[mvp_SI],
							(bRound) ? g_strRoundPlayerData[mvp_SI][team][plySIDamage] : g_strPlayerData[mvp_SI][plySIDamage],
							RoundFloat((bRound) ?
				...
				FormatEx(tmpBuffer, sizeof(tmpBuffer), "[MVP%s] 特感: \x03(无人)\x01\n", (bRound) ? "" : " - 全场");
				StrCat(printBuffer, iLen, tmpBuffer);
			}
		}
		...
		if (!(iBrevityFlags & BREV_CI)) {
			...
			if (mvp_Common > -1 &&  (bRound && g_strRoundData[g_iRound][team][rndCommon] || !bRound && g_strAllRoundData[team][rndCommon])) {
				if (iBrevityFlags & BREV_PERCENT) {
					FormatEx(tmpBuffer, sizeof(tmpBuffer), "[MVP%s] 小僵尸:\x03 %s \x01(\x05%d \x01只)\n",
							(bRound) ? "" : " - 全场",
							g_sPlayerName[mvp_Common],
							(bRound) ? g_strRoundPlayerData[mvp_Common][team][plyCommon] : g_strPlayerData[mvp_Common][plyCommon]
					);
				} else if (iBrevityFlags & BREV_ABSOLUTE) {
					FormatEx(tmpBuffer, sizeof(tmpBuffer), "[MVP%s] 小僵尸:\x03 %s \x01(\x04%i%%\x01)\n",
							(bRound) ? "" : " - 全场",
							g_sPlayerName[mvp_Common],
							RoundFloat((bRound) ?
					...
					FormatEx(tmpBuffer, sizeof(tmpBuffer), "[MVP%s] 小僵尸:\x03 %s \x01(\x05%d \x01只 [\x04%i%%\x01])\n",
							(bRound) ? "" : " - 全场",
							g_sPlayerName[mvp_Common],
							(bRound) ? g_strRoundPlayerData[mvp_Common][team][plyCommon] : g_strPlayerData[mvp_Common][plyCommon],
							RoundFloat((bRound) ?
	...
	if (!(iBrevityFlags & BREV_FF)) {
		if (mvp_FF == -1) {
			FormatEx(tmpBuffer, sizeof(tmpBuffer), "[LVP%s] 友伤: 没有任何友军伤害！\n",
					(bRound) ? "" : " - 全场"
			);
			
			StrCat(printBuffer, iLen, tmpBuffer);
		} else {
			FormatEx(tmpBuffer, sizeof(tmpBuffer), "[LVP%s] 友伤:\x03 %s \x01(\x05%d \x01伤害)\n",
					(bRound) ? "" : " - 全场",
	...
	if (bTank) {
		...
		if (g_iConsoleBufChunks > -1) {
			...
			if (fullTime)  {
				...
				strcopy(strTmp[0], s_len, "(not started)");
			}
...
void BuildConsoleBufferGeneral(bool bTeam = true, int iTeam = -1)
{
	...
	for (i = startRound; i <= g_iRound; i++) {
		...
		FormatEx(strTmp[0], s_len, "%3d. %15s", i + 1, strTmpMap);
		...
		if (g_strRoundData[i][team][rndSIKilled]) {
			FormatEx(strTmp[2], s_len, "%5d", g_strRoundData[i][team][rndSIKilled]);
		} else {
			strcopy(strTmp[2], s_len, "     ");
		}
		...
		if (g_strRoundData[i][team][rndCommon]) {
			FormatEx(strTmp[3], s_len, "%6d", g_strRoundData[i][team][rndCommon]);
		} else {
			strcopy(strTmp[3], s_len, "      ");
		}

		// deaths
		if (g_strRoundData[i][team][rndDeaths]) {
			FormatEx(strTmp[4], s_len, "%6d", g_strRoundData[i][team][rndDeaths]);
		} else {
		...
		if (g_strRoundData[i][team][rndIncaps]) {
			FormatEx(strTmp[5], s_len, "%6d", g_strRoundData[i][team][rndIncaps]);
		} else {
		...
		if (g_strRoundData[i][team][rndKitsUsed]) {
			FormatEx(strTmp[6], s_len, "%4d", g_strRoundData[i][team][rndKitsUsed]);
		} else {
		...
		if (g_strRoundData[i][team][rndPillsUsed]) {
			FormatEx(strTmp[7], s_len, "%6d", g_strRoundData[i][team][rndPillsUsed]);
		} else {
		...
		if (g_strRoundData[i][team][rndRestarts]) {
			FormatEx(strTmp[8], s_len, "%8d", g_strRoundData[i][team][rndRestarts]);
		} else {
			strcopy(strTmp[8], s_len, "        ");
		}
...
void BuildConsoleBufferSpecial(bool bRound = false, bool bTeam = true, int iTeam = -1)
{
	...
	for (x = 0; x < g_iPlayers; x++) {
		...
		if (bRound && (g_strRoundPlayerData[i][team][plySkeets] || g_strRoundPlayerData[i][team][plySkeetsHurt] || g_strRoundPlayerData[i][team][plySkeetsMelee]) ||
			...
			FormatEx(strTmp[0], s_len, "%4d /%4d /%4d",
	...
	if (details) {
		...
		for (i = 0; i < g_iPlayers; i++) {
			...
			if (bRound && g_strRoundPlayerData[i][team][plyHitsSmg] || !bRound && g_strPlayerData[i][plyHitsSmg]) {
				...
				FormatEx(strTmp[1], s_len, "%6d %5s%%%% %5d",
			...
			if (bRound && g_strRoundPlayerData[i][team][plyHitsSniper] || !bRound && g_strPlayerData[i][plyHitsSniper]) {
				...
				FormatEx(strTmp[2], s_len, "%6d %5s%%%% %5d",
			...
			if (bRound && g_strRoundPlayerData[i][team][plyHitsPistol] || !bRound && g_strPlayerData[i][plyHitsPistol]) {
				...
				FormatEx(strTmp[3], s_len, "%6d %5s%%%% %5d",
		...
		for (i = 0; i < g_iPlayers; i++) {
			...
			if (bRound && g_strRoundPlayerData[i][team][plyShotsSmg] || !bRound && g_strPlayerData[i][plyShotsSmg]) {
				...
				FormatEx(strTmp[1], s_len, "%5d %5s%%%% %5s%%%%",
			...
			if (bRound && g_strRoundPlayerData[i][team][plyShotsSniper] || !bRound && g_strPlayerData[i][plyShotsSniper]) {
				...
				FormatEx(strTmp[2], s_len, "%5d %5s%%%% %5s%%%%",
			...
			if (bRound && g_strRoundPlayerData[i][team][plyShotsPistol] || !bRound && g_strPlayerData[i][plyShotsPistol]) {
				...
				FormatEx(strTmp[3], s_len, "%5d %5s%%%% %5s%%%%",
	...
	if (bTank) {
		...
		for (x = 0; x < g_iPlayers; x++) {
			...
			if (fullTime && presTime) {
				...
				strcopy(strTmpB, s_len, "             ");
			}
			
			FormatEx(strTmp[0], s_len, "%13s %8s", strTmpB, strTmpA);
		...
		for (x = 0; x < g_iPlayers; x++) {
			...
			if (bRound && g_strRoundPlayerData[i][team][plySIDamage] || !bRound && g_strPlayerData[i][plySIDamage]) {
				...
				FormatEx(strTmp[0], s_len, "%4d %8d  %5s%s",
...
void BuildConsoleBufferFriendlyFireGiven(bool bRound = true, bool bTeam = true, int iTeam = -1)
{
	...
	for (x = 0; x < g_iPlayers; x++) {
		...
		if (!bRound && g_strPlayerData[i][plyFFGivenBullet] || bRound && g_strRoundPlayerData[i][team][plyFFGivenBullet]) {
			...
			strcopy(strPrint[FFTYPE_BULLET], s_len, "       ");
		}
		...
		if (!bRound && g_strPlayerData[i][plyFFGivenMelee] || bRound && g_strRoundPlayerData[i][team][plyFFGivenMelee]) {
			...
			strcopy(strPrint[FFTYPE_MELEE], s_len, "      ");
		}
		...
		if (!bRound && g_strPlayerData[i][plyFFGivenFire] || bRound && g_strRoundPlayerData[i][team][plyFFGivenFire]) {
			...
			strcopy(strPrint[FFTYPE_FIRE], s_len, "      "); 
		}
		...
		if (!bRound && g_strPlayerData[i][plyFFGivenIncap] || bRound && g_strRoundPlayerData[i][team][plyFFGivenIncap]) {
			...
			strcopy(strPrint[FFTYPE_INCAP], s_len, "        "); 
		}
		...
		if (!bRound && g_strPlayerData[i][plyFFGivenOther] || bRound && g_strRoundPlayerData[i][team][plyFFGivenOther]) {
			...
			strcopy(strPrint[FFTYPE_OTHER], s_len, "      "); 
		}
...
Action Timer_AutomaticRoundEndPrint(Handle hTimer)
{
	...
	// 趣文显示:
	//   有 l4d2_scripted_hud —— HUD 轮播"本回合趣文"(每条 FUNFACT_SWITCH_INTERVAL 秒, 窗口 FUNFACT_HUD_SHOW_TIME 秒),
	//                          聊天框只补一条"全场趣文"(见
...
// 池内每条 FUNFACT_SWITCH_INTERVAL 秒依次轮换(scripted_hud 的 FUNFACT_FACT_INTERVAL), 窗口 FUNFACT_HUD_SHOW_TIME 秒.
...
int DisplayFunFactHUD(int iFlags = AUTO_FUNFACT_ROUND, bool bTeam = true, int iTeam = -1)
{
	...
	FunFactLog("HUD 推送成功: %d 条本回合趣文入池, 窗口 %.1f 秒 (每条 %.1f 秒).", iPooled, FUNFACT_HUD_SHOW_TIME, FUNFACT_SWITCH_INTERVAL);
...
Action Cmd_FunFactHud(int client, int args)
{
	...
	switch (iResult) {
		case FUNFACT_HUD_OK: {
			ReplyToCommand(client, "\x04[提示]\x03已把本回合趣文推送到脚本 HUD 轮播\x05(槽位 2, 每条 %.1f 秒, 共 %.0f 秒).", FUNFACT_SWITCH_INTERVAL, FUNFACT_HUD_SHOW_TIME);
		}
...
int GetPlayerIndexForClient(int client)
{
	...
	if (IsFakeClient(client)) {
		FormatEx(sSteamId, sizeof(sSteamId), "BOT_%i", GetPlayerCharacter(client));
	} else {
...
void FormatTimeAsDuration(char[] text, int maxlength, int time, bool bPad = true)
{
	...
	if (time < 1) {
		strcopy(text, maxlength, "");
		return;
	}

	if (time > 3600) {
		int tmpHr = RoundToFloor(float(time) / 3600.0);
		FormatEx(tmp, maxlength, "%ih", tmpHr);
		time -= (tmpHr * 3600);
	}
	...
	if (time > 60) {
		...
		FormatEx(tmp, maxlength, "%s%im", (bPad && tmpMin < 10) ? " " : "" , tmpMin);
		time -= (tmpMin * 60);
	}
