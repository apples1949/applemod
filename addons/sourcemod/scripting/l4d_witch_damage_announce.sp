...
{
    CPrintToChatAll("{default}[{green}!{default}] {blue}Witch {default}还剩下 {olive}%d {default}血!", RoundToFloor(fHealthWitch) - damageWitchTotal);
}
...
void PrintWitchDamage()
{
    if (!bWitchSpawned)
    {
        CPrintToChatAll("{default}[{green}!{default}] 对{blue} Witch {default}产生的{blue}伤害");
    }
