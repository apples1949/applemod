...
{
    name = "Witch Damage Announce (tranchi)",
    author = "apples1949",
...
public void OnPluginStart()
{
    ...
    cvarCombinationName = CreateConVar("l4d_wda_combination_name", "其他生还者", "What do we call the combined players? Character limit of 32.");
...
void WitchDeath_Event(Event hEvent, const char[] name, bool dontBroadcast)
{
    ...
    {
        CPrintToChatAll("{default}[{green}!{default}] {red}坦克 {default}({olive}%N{default}) 击杀了 {red}女巫", killer);
        return;
    }
...
void PrintWitchDamage(int witch, bool witchAlive = false)
{
    ...
    if (witchAlive)
        CPrintToChatAll("{default}[{green}!{default}] {blue}女巫 {default}剩余生命 {olive}%d", witchMaxHealth - damageWitchTotal[witch]);
    else
        CPrintToChatAll("{default}[{green}!{default}] {blue}对女巫造成的伤害：");
