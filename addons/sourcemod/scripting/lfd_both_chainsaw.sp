...
public Action eventWeaponFire(Event event, const char[] name, bool dontBroadcast)
{
	if (event.GetInt("weaponid") == 20)
	{
		...
		if (!isPlayerAliveSurvivor(client)) return Plugin_Handled;

		int activeWeapon = GetEntPropEnt(client, Prop_Send, "m_hActiveWeapon");
		if (!isValidEntity(activeWeapon)) return Plugin_Handled;

		char classname[32];
		GetEntityClassname(activeWeapon, classname, sizeof(classname));
		if (!StrEqual(classname, "weapon_chainsaw")) return Plugin_Handled;
