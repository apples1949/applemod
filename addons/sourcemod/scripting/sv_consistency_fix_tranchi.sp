...
{
	name = "sv_consistency fixes (tranchi)",
	author = "apples1949",
...
Action PrintWhitelist(Handle hTimer, any iUserId)
{
	...
	if (iClient > 0) {
		...
		PrintToConsole(iClient, " ");
		PrintToConsole(iClient, "// -------------------------------- \\");
		PrintToConsole(iClient, "/| --> 欢迎来到 %s <--", sMessage);
		PrintToConsole(iClient, "|");
		PrintToConsole(iClient, "| 你的声音文件已检查完毕。");
		PrintToConsole(iClient, "| 请不要作弊。");
		PrintToConsole(iClient, "| 祝你游戏愉快。");
		PrintToConsole(iClient, "|");
		PrintToConsole(iClient, "/| --> 欢迎来到 %s <--", sMessage);
		PrintToConsole(iClient, "// -------------------------------- \\");
		PrintToConsole(iClient, " ");
...
Action Cmd_ConsistencyCheck(int iClient, int iArgs)
{
	if (iArgs < 1) {
		...
		ReplyToCommand(iClient, "已开始检查所有玩家的文件一致性！");
	...
	ReplyToCommand(iClient, "已开始检查玩家 %N (%d) 的文件一致性", iTarget, iTarget);
